...
function calcSprite(){
	...
	for(y = 0;y<21;y++){// sprite y color picker vector
	 for(x = 0;x<12;x++){// sprite x color picker vector	
		...
		n = 0;dt=0
		if (rd>142){n = n + pow(2,0);}
		if (gr>142){n = n + pow(2,1);}
		if (bl>142){n = n + pow(2,2);}			
		if (n == 1){dt=1;}
		if (n == 2){dt=2;}
		if (n == 4){dt=3;}
		if (n == 3){dt=1;}
    ...
    for (y = 0;y<21;y+=1){
		for (x = 0;x<12;x+=1){
			...
			if (bit <= 0){
				...
				if (ncnt == 8 && y < 20){pout = pout + String(sprite[scnt])+'<br>';hout = hout + hex(sprite[scnt],2)+'<br>';ncnt = 0;pout = pout + '';hout = hout + '';}
				scnt = scnt +1;
			}
			z +=1;
		}			  		
